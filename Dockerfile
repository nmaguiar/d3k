...
RUN apt-get update\
 ...
 && echo 'if [ ! -e ~/.openaf_completion_bash.sh ] || [ $(find ~/.openaf_completion_bash.sh -mtime +1) ]; then /opt/oaf/oaf --bashcompletion all > ~/.openaf_completion_bash.sh; fi' >> /etc/bash.bashrc\
